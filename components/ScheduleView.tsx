...
export function ScheduleView({ matches }: Props) {
  ...
  const hasActiveFilter = stage !== "all" || group !== "all" || team !== "all" || date !== "all";

  return (
    <section className="container-page py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">World Cup 2026 Schedule</h1>
        <p className="mt-1 text-sm text-ink-400" suppressHydrationWarning>
          All {matches.length} group-stage matches · times shown in{" "}
      ...
      <div className="mb-6 rounded-xl border border-ink-800 bg-ink-900/40 p-3">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <FilterSelect
            ...
            options={GROUPS.map((g) => ({
              label: g === "all" ? "All groups" : `Group ${g}`,
              value: g,
            }))}
          />
          <FilterSelect
        ...
        {hasActiveFilter && (
          <div className="mt-3 flex items-center justify-between text-xs text-ink-400">
            <span>
              Showing <span className="font-semibold text-ink-100">{filtered.length}</span> of{" "}
              {matches.length} matches
            </span>
            <button
              type="button"
              onClick={resetFilters}
              className="rounded-md px-2 py-1 text-pitch-400 hover:bg-ink-800 hover:text-pitch-300"
            >
              Reset filters
            </button>
          </div>
      ...
      {grouped.length === 0 ? (
        ...
        <div className="space-y-8">
          {grouped.map(({ day, matches: ms }) => (
            <div key={day}>
              <h2 className="sticky top-14 z-10 -mx-4 mb-3 bg-ink-950/85 px-4 py-2 text-sm font-semibold uppercase tracking-wider text-ink-300 backdrop-blur sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                <span suppressHydrationWarning>{formatLocalDate(`${day}T12:00:00Z`, tz)}</span>
                <span className="ml-2 text-ink-500">·</span>
                <span className="ml-2 text-ink-500">{ms.length} match{ms.length === 1 ? "" : "es"}</span>
              </h2>
