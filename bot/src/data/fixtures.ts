 ...
 */
const SPEC: ReadonlyArray<readonly [GroupLetter, 1 | 2 | 3 | 4 | 5 | 6, string, string, VenueKey]> =
  [
...
export function getLiveWindowMatches(
  now: Date = new Date(),
  windowMs = 3 * 60 * 60 * 1000,
): Match[] {
  const t = now.getTime();
  return getGroupStageMatches().filter(
    (m) => Math.abs(t - new Date(m.kickoffUTC).getTime()) < windowMs,
  );
}
